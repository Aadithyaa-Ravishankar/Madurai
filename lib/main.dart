...
class _MapPageState extends State<MapPage> with SingleTickerProviderStateMixin {
  ...
  // Add this field to track the current locale
  ...
  void initState() {
    ...
    // Initialize locale from current context
  ...
  Future<void> _loadGeoJSON() async {
    debugPrint('[_MapPageState] Starting _loadGeoJSON');
    try {
      final String jsonString = await rootBundle.loadString('assets/madurai_wards.geojson');
      debugPrint('[_MapPageState] Loaded madurai_wards.geojson');
      final Map<String, dynamic> jsonData = json.decode(jsonString);
      final List<dynamic> features = jsonData['features'];
      debugPrint('[_MapPageState] Found ${features.length} GeoJSON features');
      ...
      for (int i = 0; i < wardPoints.length; i++) {
        try {
          ...
          final label = Marker(
            ...
            onTap: () {
              debugPrint('[_MapPageState] Marker tapped for ward index $i');
              _showWardDetails(i);
            },
          );
          loadedLabels.add(label);
        } catch (e) {
          debugPrint("Error creating polygon or marker for ${wardNames[i]}: $e");
        }
      }
      debugPrint('[_MapPageState] Created polygons and markers');
      ...
      if (_mapService.mapController != null) {
        ...
        debugPrint('[_MapPageState] Waiting for map render');
        await Future.delayed(const Duration(milliseconds: 1000));
        debugPrint('[_MapPageState] Map render wait complete');
      }
      ...
      if (mounted) {
        ...
        debugPrint('[_MapPageState] Set loading to false');
      }
  ...
  Future<void> _loadCouncillorHtml() async {
    debugPrint('[_MapPageState] Starting _loadCouncillorHtml');
    try {
      debugPrint('[_MapPageState] Loading councillor HTML...');
      final String htmlContent = await rootBundle.loadString('assets/councillor.html');
      _councillorHtml = htmlContent;
      debugPrint('[_MapPageState] Successfully loaded councillor HTML');
      debugPrint('[_MapPageState] HTML content length: ${htmlContent.length}');
  ...
  void _parseCouncillorData() {
    debugPrint('[_MapPageState] Starting _parseCouncillorData');
    if (_councillorHtml == null) {
      debugPrint('[_MapPageState] Councillor HTML is null, cannot parse data');
      return;
    }

    debugPrint('[_MapPageState] Parsing councillor data...');
    final document = htmlParser.parse(_councillorHtml!);
    final rows = document.querySelectorAll('table tr');
    debugPrint('[_MapPageState] Found ${rows.length} rows in councillor table');
    ...
    for (var row in rows) {
      ...
      if (cells.length >= 8) {
        ...
        debugPrint('[_MapPageState] Parsed ward $wardNo: $name');
    ...
    debugPrint('[_MapPageState] Finished parsing councillor data. Total entries: ${_councillorData.length}');
    debugPrint('[_MapPageState] Available ward numbers: ${_councillorData.keys.join(', ')}');
  }
  ...
  void _showWardDetails(int wardIndex) {
    ...
    debugPrint('[_MapPageState] Processing ward index $wardIndex with ward number $wardNo');
    ...
    String councillorParty;
    String councillorResponsibility;
    String councillorAddress = _councillorData[wardNo]?['address'] ?? '';
    String councillorContact = _councillorData[wardNo]?['contact'] ?? '';
    String councillorEmail = _councillorData[wardNo]?['email'] ?? '';
    ...
    debugPrint('[_MapPageState] Retrieved ward data for ward number $wardNo: ${wardInfo != null ? 'Found' : 'Not Found'}');
    if (wardInfo == null) {
      debugPrint('[_MapPageState] Warning: No ward data found for ward number $wardNo');
      // Create default ward data if not found
      wardInfo = WardData(
        name: wardName,
        code: 'MDR${wardNo.padLeft(3, '0')}',
        areas: [wardName],
        facilities: ['Basic Amenities'],
        population: 'N/A',
        description: wardDescription.isNotEmpty ? wardDescription : 'No description available',
      );
    }
                    ...
                    child: Column(
                      ...
                      children: [
                        if (councillorName != 'No councillor details available') ...[
                          _buildSectionHeader(AppLocalizations.of(context)!.dynamicLookup('councillorInformationHeader') ?? 'Councillor Information'),
              ...
              content: Column(
                ...
                children: [
                  ListTile(
                    ...
                    onTap: () {
                      if (_currentLocale.languageCode != 'en') {
                        ...
                        _toggleLanguage(const Locale('en'));
                      }
                  ...
                  ListTile(
                    ...
                    trailing: _currentLocale.languageCode == 'ta'
                        ? const Icon(Icons.check_circle, color: Colors.red)
                        : const Icon(Icons.circle_outlined, color: Colors.grey),
                    onTap: () {
                      if (_currentLocale.languageCode != 'ta') {
                        ...
                        _toggleLanguage(const Locale('ta'));
                      }
                      Navigator.pop(context);
                    },
                  ),
  ...
  void _toggleLanguage(Locale newLocale) {
    // Find the nearest _MyAppState and update its locale
