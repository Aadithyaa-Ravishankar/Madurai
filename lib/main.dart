...
class _MapPageState extends State<MapPage> with SingleTickerProviderStateMixin {
  ...
  Future<void> _searchLocation(String address) async {
    // Clear existing search markers before starting a new search
    _mapService.clearSearchMarkers();
    ...
    try {
      ...
      if (data["status"] == "OK" && data["results"].isNotEmpty) {
        ...
        for (int i = 0; i < _mapService.polygons.length; i++) {
          if (_isPointInPolygon(searchLocation, _mapService.polygons.elementAt(i).points)) {
            wardNumber = _wardNumbers[i];
            wardName = _wardNames[i];
            isInMadurai = true;
            print('Found ward: $wardNumber - $wardName');
            break;
          }
        }

        if (!isInMadurai) {
          // Show message that location is outside Madurai
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Row(
                children: [
        ...
        // Create custom marker icon with info window
        ...
        final marker = Marker(
          ...
          icon: customIcon,
          anchor: const Offset(0.5, 1.0), // Anchor at bottom center of the icon
          zIndex: 1000.0, // Add high z-index to ensure it appears above other markers
        );
        ...
        if (_mapController != null) {
          print('Moving camera to: $searchLocation');
          await _mapController!.animateCamera(
            CameraUpdate.newLatLngZoom(searchLocation, 15),
          );
        } else {
          print('Error: Map controller is null');
        }

        _removeOverlay();
      } else {
        print('Geocoding API error: ${data["status"]} - ${data["error_message"]}');
        ScaffoldMessenger.of(context).showSnackBar(
  ...
  Future<void> _fetchAddressSuggestions(String query) async {
    ...
    print('Fetching address suggestions for query: $query');
    print('URL: $url');
    try {
      final response = await http.get(Uri.parse(url));
      final data = jsonDecode(response.body);
      ...
      if (data["status"] == "OK") {
        setState(() {
          _searchSuggestions = List<String>.from(
